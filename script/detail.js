...
apiready = function() {
    ...
    var articleAuthorId = '';
    var detailSet = $api.dom('.detail-set');
        ...
        apiAjax($gurl.article + '/' + id, 'GET', {}, false, false, function(res) {
            if (res && res.id) {
                ...
                if(res.user_id == userId) {
                  $api.removeCls(detailSet, 'hide');
                } else {
                  $api.addCls(detailSet, 'hide');
                }
                //获取用户信息
                getUser(res.user_id);
                //获取阅读数量
    ...
    //打开url
    function openArtUrl() {
        var links = $api.domAll('.article-detail-content a');
        openBrowserUrl(links);
    }

    //点击弹出菜单
    $api.addEvt(detailSet, 'click', openMenu, false);
    ...
    //文章属性
    //获取文章
    function updateAttrArticle(data) {
        showProgress();
        apiAjax($gurl.article + '/' + articleId, 'PUT', data , false, false, function(res) {
            if (res && res.id) {
                api.toast({
