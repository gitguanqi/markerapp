...
apiready = function() {
    ...
    //判断哪里来
    if ((from == 'detail' || 'good_list' || 'reply_list' || 'focus_list') && userIdDetail) {
    ...
    function getFocusNum(id) {
        apiAjax($gurl.focus + '?filter={"where":{"focus_user_id":"' + id + '"}}', 'GET', {}, false, false, function(res) {
            if (res) {
    ...
    function getReplyNum(id) {
        apiAjax($gurl.reply + '?filter={"where":{"reply_user_id":"' + id + '"}}', 'GET', {}, false, false, function(res) {
            if (res) {
                res = uniqObj(res, 'author_id');
                for (var i = 0; i < res.length; i++) {
                    if (res[i].author_id == id) {
                        res.splice(i, 1);
                    }
                }
...
//文章操作
function openInfoWin(id, isPrivate) {
    if (isLoginUser === authorId) {
        api.actionSheet({
                title: '文章操作',
                cancelTitle: '取消操作',
